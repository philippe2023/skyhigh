...
export default function FlightFilter({ flights, destination, flightCategory }) {
  ...
  const [flightsToShow, setFlightsToShow] = useState(
    flights.filter((flight) =>
      combinedFilter(flight, selectedDestination, selectedCategory)
    )
  );
  ...
  const handleFlightCategoryChange = (e) => {
    ...
    setFlightsToShow(
      flights.filter((flight) =>
        combinedFilter(flight, selectedDestination, category)
      )
    );
  };
  ...
  return (
    <div>
        <h1 className="text-2xl font-semibold text-center text-gray-800 lg:text-3xl dark:text-white">
          Available Flights
          {selectedDestination && ` for ${selectedDestination}`}
        </h1>
      ...
      <div className="form-control">
        <label className="label">
          <span className="label-text text-2xl font-semibold text-center text-gray-800 lg:text-3xl dark:text-white">Destination</span>
          <input
            type="text"
            ...
            onChange={(e) => {
              ...
              setFlightsToShow(
                flights.filter((flight) =>
                  combinedFilter(flight, e.target.value, selectedCategory)
                )
              );
            }}
