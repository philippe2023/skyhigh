...
function getFlightData(id) {
    const flight = db.prepare("SELECT departure, destination, price, STRFTIME('%Y-%m-%d', departure_date, 'unixepoch') AS date,max_seats, available_seats, flight_number from empty_leg WHERE id=?").get(id);
    return flight;
}
                ...
                <div className=" lg:flex">
                    <div className="w-full lg:w-1/2">
                        <div className="lg:max-w-lg">
                            <h1 className="text-xl font-semibold text-gray-800 dark:text-white lg:text-2xl">Empty Leg by <span className="text-blue-500 ">FlyAway LLC</span></h1>
                            <p className="mt-3 text-gray-600 dark:text-gray-400">{flightData.available_seats} {flightData.available_seats === 1 ? 'Seat' : 'Seats'} available - 1 Flight attendant - 2h Flight</p>
                    ...
                    <div className="flex justify-end w-full mt-6 lg:mt-0 lg:w-1/2 px-4">
                        <form className="px-4 mx-4 border-solid border-4 border-slate-200 rounded-lg">
                            ...
                            <div className="m-4 mb-8 py-2 px-4 border-solid border-4 border-slate-400 rounded-lg">
                                ...
                                <input type="number" min="1" max="5" name="GUESTS" placeholder="How many?" required size="4" minength="4" className="container mx-auto text-black text-base" />
                            </div>
